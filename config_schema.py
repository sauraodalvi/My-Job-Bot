...
AI_MODELS = {
    ...
    "openrouter": [
        "openrouter/auto", "openai/gpt-5-mini", "openai/gpt-4o-mini",
...
SCHEMA = [
    {
        ...
        "fields": [
            ...
            _f("Account", "secrets", "ai_provider", "AI provider", "select",
               "Which AI service to use. Pick 'openai' for OpenAI or any OpenAI-compatible service (including local ones like Ollama or LM Studio), 'deepseek' for DeepSeek, 'openrouter' for OpenRouter (one key, many models), or 'gemini' for Google Gemini.",
               options=["openai", "deepseek", "openrouter", "gemini"], ai=True),
            ...
            _f("Account", "secrets", "llm_api_key", "AI API key", "password",
               "Your AI service API key for OpenAI / DeepSeek / local models. Use 'not-needed' for local models like Ollama. A wrong key causes errors while AI is on.",
               ai=True),
            # --- dedicated keys per provider (fill one and switch freely) ---
            _f("Account", "secrets", "gemini_api_key", "Gemini API key", "password",
               "Your Google Gemini API key (get one free at https://aistudio.google.com/apikey). Used only when the provider is 'gemini'. Leave blank to use the AI API key above; Google's GOOGLE_API_KEY or GEMINI_API_KEY environment variable also works.",
               ai=True),
            _f("Account", "secrets", "openrouter_api_key", "OpenRouter API key", "password",
               "Your OpenRouter API key (get one at https://openrouter.ai/keys). Used only when the provider is 'openrouter'. Leave blank to use the AI API key above or set the OPENROUTER_API_KEY environment variable.",
               ai=True),
            # --- advanced AI plumbing ---
            _f("Account", "secrets", "llm_api_url", "AI API URL", "text",
               "The address of your AI service. Examples: https://api.openai.com/v1/ , http://localhost:1234/v1/ , https://api.deepseek.com . Keep the trailing slash. OpenRouter switches to https://openrouter.ai/api/v1 automatically; you may not need this for Gemini.",
    ...
    },
    {
        "section": "Filters",
        "fields": [
            _f("Filters", "search", "skip_companies", "Skip these companies by name", "list",
               "Skip ALL jobs from these exact company names, comma-separated. This is a hard block on the company name shown on the job card - it works even when a company's 'About' section is empty or doesn't mention the offending words."),
            _f("Filters", "search", "skip_resume_companies", "Skip companies already on my resume", "bool",
               "Automatically skip jobs at the companies found on your resume (past and current employers)."),
    ...
    {
        "section": "Referral Messaging",
        "fields": [
            _f("Referral Messaging", "settings", "send_referral_dms", "Send referral messages", "bool",
               "Master switch for auto-sending referral messages. When on, the bot can send personalized referral requests via LinkedIn DM and/or Gmail to HR contacts found during the referral scan."),
            _f("Referral Messaging", "settings", "send_via_linkedin", "Send via LinkedIn DM", "bool",
               "Send referral messages as LinkedIn direct messages to the HR contact's profile."),
            _f("Referral Messaging", "settings", "send_via_gmail", "Send via Gmail", "bool",
               "Send referral messages as Gmail emails. Requires you to be logged into Gmail in the bot's browser. Only works when the HR contact's email is discoverable on their LinkedIn profile."),
            _f("Referral Messaging", "settings", "linkedin_dm_template", "LinkedIn DM template", "textarea",
               "Message sent as a LinkedIn DM. Use {variable} placeholders: {employee_name}, {job_title}, {company_name}, {job_link}, {your_name}, {your_role}, {your_company}, {your_linkedin}, {your_portfolio}."),
            _f("Referral Messaging", "settings", "gmail_subject", "Gmail subject line", "text",
               "Subject line for referral emails. Use the same {variable} placeholders as the body template."),
            _f("Referral Messaging", "settings", "gmail_body", "Gmail body template", "textarea",
               "Body of the referral email. Use {variable} placeholders: {employee_name}, {job_title}, {company_name}, {job_link}, {your_name}, {your_role}, {your_company}, {your_linkedin}, {your_portfolio}."),
            _f("Referral Messaging", "settings", "referral_dm_delay", "Delay between messages (seconds)", "number",
               "How many seconds to wait between sending messages (randomized for anti-detection). Higher is safer. Recommended 45-60."),
            _f("Referral Messaging", "settings", "referral_dm_max", "Max messages per run", "number",
               "Maximum number of messages to send per channel per run. Capped to avoid LinkedIn/Gmail rate limits."),
        ],
    },
    {
